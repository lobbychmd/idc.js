...
var validateType = {
    last_update: { //value, parent, doc, type, _id
        message: '数据已经更改过，请重新获取', validate: function (dataParams, invokeParams, callback) {
            var m = mongoose.model(dataParams.type);
            m.findOne({ _id: dataParams._id }, function (err, doc1) {
                if (doc1) {
                    callback((!dataParams.parent.LastUpdateTime && !doc1.LastUpdateTime) || (new Date(dataParams.parent.LastUpdateTime).toUTCString() == doc1.LastUpdateTime.toUTCString()));
                }
                else callback(true);
            });
