    ...
    if (q == null) return null;
        ...
        while (r) {
            ...
            if (subDoc) {
                ...
                for (j in v) result.push(v[j][q.substring(1).split('.')[1]]);
                return result;
            }
...
exports.inherite = function (query, doc, callback) {
    if (doc) callback(doc);
        ...
        var parent_type = parent_type.indexOf('.') > 0 ? parent_type.split('.')[0] : parent_type;
        ...
        if (inherite) {
            if (!tree_config[parent_type].table) parent_type = tree_config[parent_type].parent;
