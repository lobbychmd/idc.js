...
exports.add_version = function (type, _id, callback) {
    ...
    m.findOne({ _id: _id }, function (err, doc) {
        ...
        mv.where("MetaType", type).where("ModelId", _id).desc('Version').run(function (err1, verdocs) {
            console.log(verdocs[0].Version);
            var newver = verdocs.length > 0 && verdocs[0] && verdocs[0].Version ? parseInt(verdocs[0].Version) + 1 : 999000;
            if (newver < 999000) newver = 999000 + newver;
            console.log(newver);
            doc.HashCode = null;
            doc.Version = newver;
